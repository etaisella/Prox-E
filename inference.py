...
from prox_e.inversion import run_full_inversion, load_trellis_text_pipeline
...
def get_category_samples(category_name):
    ...
    if category_name not in SHAPENET_CATEGORIES:
        ...
        raise ValueError(
            f"Category '{category_name}' not found. Available: {', '.join(sorted(available))}"
        )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate superquadric abstractions from ShapeNet models"
    )
    parser.add_argument(
        "--output_folder",
        type=str,
    ...
    parser.add_argument(
        "--edit_instruction",
        type=str,
        default="Make the seat twice as thick",
...
def editing_pipeline(
    args, sample_path: Path, sample_id: str, text_pipeline=None, image_pipeline=None
):
    """
    ...
    if sample_path.is_file():
        ...
        can_reuse_normalized = normalized_mesh_path.exists() and (
            ...
            or (
                args.orientation_index is None
                and not orientation_metadata_path.exists()
            )
        )
        if not can_reuse_normalized:
            ...
            print(
                f"  Original center: {norm['center']}, max_dim: {norm['max_dim']:.4f}"
            )
    ...
    vlm_inputs_exist = (sample_output_folder / "vlm_original.png").exists() and (
        sample_output_folder / "vlm_abstraction.png"
    ).exists()
    ...
    if abstraction_json_exists and vlm_inputs_exist:
        ...
        render_obj_with_blender(
            str(mesh_file),
            str(sample_output_folder / "original.png"),
            shade_smooth=True,
        )
        ...
        render_obj_with_blender(
            result["obj_path"], str(sample_output_folder / "original_abstraction.png")
        )
        render_multiview(
            result["obj_path"], sample_output_folder, single_view=args.single_view
        )
        ...
        print("Original render:  original.png")
        print("SuperDec JSON:    superdec.json")
        print("SuperDec OBJ:     superdec.obj")
        print("Abstraction JSON: abstraction.json")
        print("VLM inputs:       vlm_original.png, vlm_abstraction.png")
    ...
    if edit_instruction_file.exists():
        ...
        if not instruction_matches:
            ...
            print(
                f"  Saved:   {saved_instruction[:60]}{'...' if len(saved_instruction) > 60 else ''}"
            )
            print(
                f"  Current: {args.edit_instruction[:60]}{'...' if len(args.edit_instruction) > 60 else ''}"
            )
        else:
    ...
    cached = (
        load_parse_result(parse_result_path, category) if instruction_matches else None
    )
    ...
    if cached:
        ...
        structural_description = (
            parse_result["structural_description"] or f"a {category}"
        )
        appearance_description = (
            parse_result["appearance_description"] or f"a {category}"
        )
    ...
    if vlm_outputs_exist and not edited_obj_path.exists():
        ...
        edited_mesh = mesh_from_abstraction(
            edited_abstraction_data, resolution=RESOLUTION
        )
    ...
    if no_structural_changes:
        ...
        original_mesh = mesh_from_abstraction(
            original_abstraction_data, resolution=RESOLUTION
        )
        ...
        render_obj_with_blender(
            str(edited_obj_path), str(sample_output_folder / "edited_final_render.png")
        )
        ...
        print("\n" + "=" * 80)
        print(
            f"Editing shape with VLM ({args.vlm}) - max {args.max_iterations} iterations..."
        )
        print("=" * 80)
    ...
    if (
        merging_data_path.exists()
        and latent_masks_folder.exists()
    ...
    inversion_latents_path = (
        sample_output_folder / "inversion" / "original_shape_ss_latents.pt"
    )
    abs_inversion_latents_path = (
        sample_output_folder / "inversion" / "abstraction_ss_latents.pt"
    )
    slat_latents_path = (
        sample_output_folder / "inversion" / "original_shape_slat_latents.pt"
    )
    ...
    inversion_exists = (
        inversion_latents_path.exists()
        and abs_inversion_latents_path.exists()
        and slat_latents_path.exists()
    )
    if inversion_exists and instruction_matches:
        print("\n" + "=" * 80)
        print(
            "SKIPPING INVERSION: Latents already exist for original shape and abstraction"
        )
        print("=" * 80)
        print(
            f"  To re-run, delete: {inversion_latents_path} and {abs_inversion_latents_path}"
        )
        ...
        print("  Using unified normalization for appearance editing")
    else:
        print("\n" + "=" * 80)
        ...
        features_exist = (
            ...
            / "inversion"
            / "original_shape_renders"
            / "features.npz"
        ).exists()

        if slat_latents_exist:
            print(f"  SLAT latents already exist: {slat_latents_path}")
            print("  Skipping rendering and feature extraction")
        elif features_exist:
            print("  Features already exist, skipping multi-view rendering")
            print("  Will re-do SLAT inversion (conditioning may have changed)")
        ...
        normalization_data = (
            unified_norm  # Use unified normalization (same as passed in)
        )
    ...
    if merging_data is not None and "changed_edited_masks" in merging_data:
        for mask_data in merging_data["changed_edited_masks"]:
            ...
            latent_path = (
                sample_output_folder
                / "inversion"
                / f"transformed_sq{sq_index}_ss_latents.pt"
            )
            if latent_path.exists():
                ...
                print(
                    f"Warning: Transformed mesh latent not found for SQ {sq_index}: {latent_path}"
                )
    ...
    if inpainted_voxels_path.exists() and instruction_matches:
        ...
        original_voxels_path = (
            sample_output_folder / "inversion" / "original_shape_voxels.pt"
        )
        if original_voxels_path.exists():
            ...
            raise FileNotFoundError(
                f"Original voxels not found: {original_voxels_path}"
            )
        save_voxel_artifacts(
            voxels,
            sample_output_folder,
        ...
        voxels = run_inpainting(
            ...
            render_fn=render_obj_with_blender,
            merging_data=merging_data,
            transformed_mesh_latents=transformed_mesh_latents
            if transformed_mesh_latents
            else None,
            normalization=normalization_data,
    ...
    if args.appearance_condition_path:
        ...
        if not custom_condition_path.exists():
            raise FileNotFoundError(
                f"Appearance condition path not found: {custom_condition_path}"
            )
        ...
        preprocessed_image.save(
            sample_output_folder / "conditioning_render_preprocessed.png"
        )
        ...
        preprocessed_image = preprocess_single_image(
            str(edited_condition_path), skip_rembg=True
        )
        preprocessed_image.save(
            sample_output_folder / "conditioning_render_preprocessed.png"
        )
        ...
        original_inversion_data is not None and "image_cond" in original_inversion_data
    ):
        ...
        if not conditioning_render_path.exists():
            raise FileNotFoundError(
                f"No image conditioning available. conditioning_render.png not found: {conditioning_render_path}"
            )
    ...
    appearance_steps = (
        APPEARANCE_INJECTION_STEPS_W_EDIT
        if use_custom_appearance
        else args.appearance_injection_steps
    )
...
if __name__ == "__main__":
    ...
    if args.input_mesh:
        ...
        if input_mesh_path.is_file():
            sample_id = (
                input_mesh_path.stem
            )  # Use filename without extension as sample_id
