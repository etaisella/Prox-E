...
from .models.mm_utils import extract_frames, concatenate_images_horizontal
...
class Score(nn.Module):
    def __init__(
        self, model: str, device: str = "cuda", cache_dir: str = HF_CACHE_DIR, **kwargs
    ):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if any(
            isinstance(img, str) and img[-4:].lower() in valid_video_extensions
            for img in images
        ):
            if self.model.video_mode == "concat":
                ...
                for video in images:
                    ...
                    if isinstance(video, str):
                        ...
                        frame_images = [
                            cv2.imread(os.path.join(output_dir, f))
                    ...
                    concat_image = concatenate_images_horizontal(
                        frame_images, dist_images=10
                    )
                ...
                print(
                    "Invalid `video_mode` for the given model. Please check model's class attributes"
                )
                return
    ...
    def forward_multi_images(
        ...
        **kwargs,
        ...
        for i, image in enumerate(processed_images):
            if not parse_answer:
                scores[i] = self.model.forward_multi_images(
                    [image] * len(texts), texts, parse_answer=parse_answer, **kwargs
                )
            else:
                outputs = self.model.forward_multi_images(
                    [image] * len(texts), texts, parse_answer=parse_answer, **kwargs
                )
                scores[i] = outputs[0]
                out_text = outputs[1]
            ...
            for vis_idx in range(num_visuals):
                ...
                for text_idx in range(num_texts):
                    ...
                    if media_type == "videos":
                        scores[
                            counter : counter + cur_batch_size, vis_idx, text_idx
                        ] = torch.squeeze(
                            torch.cat(
                                [
                                    self.forward(
                                    ...
                                    for (visual, text) in zip(visuals, texts)
                                ],
                                dim=0,
                            )
                        )
                        ...
                        scores[
                            ...
                            torch.cat(
                                [
                                    self.forward(images=visual, texts=text, **kwargs)
                                    for (visual, text) in zip(visuals, texts)
                                ],
                                dim=0,
                            )
