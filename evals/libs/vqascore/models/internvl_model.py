...
INTERNVL2_MODELS = {
    ...
    "internvl3_5-8b": {
        "tokenizer": {
            "path": "OpenGVLab/InternVL3_5-8B-Instruct",
            "trust_remote_code": True,
            "use_fast": False,
        },
        "model": {
            "pretrained_model_name_or_path": "OpenGVLab/InternVL3_5-8B-Instruct",
            "torch_dtype": torch.bfloat16,
            "low_cpu_mem_usage": True,
            "use_flash_attn": True,
            "trust_remote_code": True,
    ...
    "sensenova-si-internvl3-8b": {
        "tokenizer": {
            "path": "sensenova/SenseNova-SI-1.3-InternVL3-8B",
            "trust_remote_code": True,
            "use_fast": False,
        },
        "model": {
            "pretrained_model_name_or_path": "sensenova/SenseNova-SI-1.3-InternVL3-8B",
            "torch_dtype": torch.bfloat16,
            "low_cpu_mem_usage": True,
            "use_flash_attn": True,
            "trust_remote_code": True,
...
class InternVL2Model(VQAScoreModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert model_name in INTERNVL2_MODELS, (
        ...
        if checkpoint:
            self.model_info = {
                "tokenizer": {**self.model_info["tokenizer"], "path": checkpoint},
                "model": {
        ...
        if "sensenova" in self.model_name:
            self.max_num_per_image = 6
        else:
            self.max_num_per_image = 12
    ...
    def dynamic_preprocess(
        self, image, min_num=1, max_num=-1, image_size=448, use_thumbnail=False
    ):
        if max_num == -1:
    ...
    def forward_multi_images(
        self,
        paths: List[Union[str, List[str]]],
        texts: List[str],
        num_frames: int = 16,
        question_template: str = 'Does the images show "{}"?',
        answer_template: str = "Yes",
        ...
        for path_input, question, answer in zip(paths, questions, answers):
            # 1. Handle Input Grouping
            if isinstance(path_input, str):
                current_paths = [path_input]
            else:
                current_paths = path_input

            # 2. Load images for THIS specific prompt only
            data, num_patches = self.load_images(current_paths, num_frames)

            # InternVL handles multi-image by treating them as video frames
            ...
            # 3. FIX: Concatenate list of tensors into one tensor
            # data is [Tensor_Img1, Tensor_Img2], we need Tensor_Stacked
            pixel_values = torch.cat(data, dim=0).to(self.device)
            pixel_values = pixel_values.to(self.model.dtype)
            ...
            if not parse_answer:
                scores = outputs.scores[0]
                probs = torch.nn.functional.softmax(scores, dim=-1)
                yes_token_id = self.tokenizer.encode(answer, add_special_tokens=False)[
                    0
                ]
                lm_prob = probs[0, yes_token_id].item()
            else:
                ...
                yes_id_space = self.tokenizer.encode(" Yes", add_special_tokens=False)[
                    0
                ]
                no_id_space = self.tokenizer.encode(" No", add_special_tokens=False)[0]
    ...
    def generate(
        ...
        gen_outputs = []
        for data, question, num_patches in zip(
            processed_data, questions, num_patches_list
        ):
            if isinstance(num_patches, list):  # Video
                video_prefix = "".join(
                    [f"Frame{i + 1}: <image>\n" for i in range(len(num_patches))]
