...
def concatenate_images_grid(images, dist_images, output_size):
    ...
    for index, img in enumerate(images):
        ...
        new_img[y_offset : y_offset + new_height, x_offset : x_offset + new_width] = (
            resized_img
        )
...
def load_pretrained_model(
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        tokenizer_path, use_fast=False, **tokenizer_dict
    )
    ...
    if mmprojector_repo:
        ...
        model_args.pretrain_mm_mlp_adapter = (
            pretrain_mm_mlp_adapter  # important to set to correct path
        )
