...
def merge_latents_with_masks(
    ...
    if t_key not in inverted_latents:
        print(
            f"[LatentMerging] Step {step_idx}: No inverted latent for t={timestep:.6f}"
        )
        return original_sample, False
    ...
    mask_changed_original_dilated = dilate_mask_3d(
        changed_original_mask, dil_changed_original
    )
    ...
    use_original_mask = torch.from_numpy(
        np.logical_or(mask_added_deleted_dilated, mask_changed_original_dilated).astype(
            np.float32
        )
    ...
    if changed_edited_masks and len(changed_edited_masks) > 0:
        ...
        use_edited_mask = torch.from_numpy(changed_edited_union.astype(np.float32)).to(
