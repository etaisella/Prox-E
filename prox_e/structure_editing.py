...
VLM_FEEDBACK_INSTRUCTION_PATH = REPO_ROOT / "instruction_prompts" / "vlm_feedback_instruction.txt"
...
def prepare_vlm_input(
    ...
    if not skip_original:
        print(f"VLM input images saved:")
...
def _create_labeled_grid(
    ...
    positions = [
        ...
        (padding * 2 + img_width, title_height + padding * 2 + cell_height),  # bottom-right: back
    ]
...
def query_gemini(
    ...
    if not vlm_abstraction_path.exists():
        raise FileNotFoundError(f"VLM abstraction image not found: {vlm_abstraction_path}")
    ...
    messages = [
        {
            ...
            "content": [
                ...
                {"type": "text", "text": "Here is the abstracted shape from multiple views:"},
...
def _load_vlm_instruction() -> str:
    ...
    if not VLM_INSTRUCTION_PATH.exists():
        raise FileNotFoundError(f"VLM instruction file not found: {VLM_INSTRUCTION_PATH}")
...
Please analyze the shape and provide the edited JSON.""",
    ...
    if use_cache:
        print(f"Creating Gemini context cache...")
        try:
            ...
            print(f"  Falling back to non-cached query...")
            cache = None
    ...
    if not use_cache or cache is None:
        ...
        full_contents = [system_instruction] + cache_contents + [
             "\nPlease analyze the shape and provide the edited JSON."
        ]
    ...
    if result.usage_metadata:
        ...
        token_usage['cached_tokens'] = result.usage_metadata.cached_content_token_count or 0
        
        print(f"  Tokens: {token_usage['input_tokens']} in "
...
def _query_gemini_verify(cached_model, edited_img_path: Union[str, Path],
                         previous_response: str, model_name: str = "gemini-2.0-flash") -> Dict:
...
{_load_vlm_verify_instruction()}"""
    ...
    if hasattr(result, 'usage_metadata') and result.usage_metadata:
        token_usage['input_tokens'] = getattr(result.usage_metadata, 'prompt_token_count', 0) or 0
        token_usage['output_tokens'] = getattr(result.usage_metadata, 'candidates_token_count', 0) or 0
        token_usage['cached_tokens'] = getattr(result.usage_metadata, 'cached_content_token_count', 0) or 0
        print(f"  Tokens: {token_usage['input_tokens']} in ({token_usage['cached_tokens']} cached), {token_usage['output_tokens']} out")
...
def edit_shape_with_vlm(
    ...
    use_gemini_cache = use_gemini_cache and vlm.lower() == "gemini" and max_iterations > 1
    ...
    try:
        ...
        if current_abstraction is None:
            ...
            history.append({'iteration': 0, 'type': 'initial_edit', 'status': 'parse_error', 'response': output_text})
        else:
            print(f"Parsed initial edit ({len(current_abstraction)} primitives)")
            history.append({'iteration': 0, 'type': 'initial_edit', 'status': 'ok', 'response': output_text})
            ...
            if max_iterations == 1:
                ...
                for iteration in range(1, max_iterations + 1):
                    ...
                    print(f"Rendering current abstraction ({len(current_abstraction)} primitives)...")
                    ...
                    render_multiview(edited_obj_path, output_folder, prefix=f"edited_iter{iteration}_")
                    ...
                    if _check_if_done(feedback_response):
                        ...
                        success = True
                        history.append({'iteration': iteration, 'type': 'feedback_fix', 'status': 'done', 'response': feedback_response})
                        break
                    ...
                    if new_abstraction is None:
                        print("Warning: Could not parse JSON from feedback, keeping previous version")
                        history.append({'iteration': iteration, 'type': 'feedback_fix', 'status': 'parse_error', 'response': feedback_response})
                        # Continue to next iteration with unchanged abstraction
                    else:
                        print(f"Parsed fix from feedback ({len(new_abstraction)} primitives)")
                        current_abstraction = new_abstraction
                        final_response = feedback_response  # Use this as context for next iteration
                        history.append({'iteration': iteration, 'type': 'feedback_fix', 'status': 'fixed', 'response': feedback_response})
                        
                        # Save iteration JSON
                        iter_json_path = output_folder / f"edited_abstraction_iter{iteration}.json"
        ...
        if history:
            ...
            with open(history_path, 'w') as f:
                json.dump([{
                    ...
                    'status': h['status'],
                    'response_preview': h['response'][:500] + "..." if len(h['response']) > 500 else h['response'],
        ...
        for entry in token_usage_log:
            ...
            print(f"    Iter {entry['iteration']} ({entry['type']}): {entry['input_tokens']:,} in, {entry['output_tokens']:,} out{cached_str}")
...
def _load_vlm_feedback_instruction() -> str:
    ...
    if not VLM_FEEDBACK_INSTRUCTION_PATH.exists():
        raise FileNotFoundError(f"VLM feedback instruction file not found: {VLM_FEEDBACK_INSTRUCTION_PATH}")
        ...
        messages = [
            {
                ...
                "content": [
                    ...
                    {"type": "image_url", "image_url": {"url": f"data:image/png;base64,{abstraction_b64}"}},
...
def _check_if_done(response_text: str) -> bool:
    ...
    return "STATUS: DONE" in response_text.upper() or "STATUS:DONE" in response_text.upper()
