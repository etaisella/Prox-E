...
def remove_floaters(
    voxels: torch.Tensor, dilation_steps: int = 2, threshold: float = 0.5
) -> torch.Tensor:
    """
    ...
    print(
        f"  Floater removal: {num_features} components found, kept main ({component_sizes[0][1]} voxels in dilated), removed {removed_count} floater voxels"
    )

    # Convert back to tensor
    filtered_tensor = (
        torch.from_numpy(filtered_voxels).unsqueeze(0).unsqueeze(0).to(voxels.device)
    )
...
class InpaintingSampler(InversionFlowEulerGuidanceIntervalSampler):
    ...
    def sample_inpainting(
        ...
        if edit_dilation_steps > 0:
            added_deleted_mask_np = binary_dilation(
                added_deleted_mask_np > 0.5, iterations=edit_dilation_steps
            ).astype(np.float32)
        ...
        if edit_dilation_steps > 0:
            changed_original_mask_np = binary_dilation(
                changed_original_mask_np > 0.5, iterations=edit_dilation_steps
            ).astype(np.float32)
        ...
        changed_original_mask = (
            torch.from_numpy(changed_original_mask_np).float().cuda()
        )
        ...
        if inject_original:
            ...
            if unchanged_mask_np is not None:
                ...
                print(
                    f"  [inject_original] Unchanged mask: {int(unchanged_mask.sum().item())} voxels"
                )
            else:
                print(
                    "  [inject_original] Warning: unchanged_mask not found in merging_data"
                )
        ...
        for i, (t_curr, t_prev) in enumerate(tqdm(t_pairs, desc="Inpainting")):
            ...
            if i < num_inpainting_steps:
                ...
                print(
                    f"Num voxels kept {keep_mask.sum().item()} out of {sample.shape[-1] * sample.shape[-2] * sample.shape[-3]}"
                )

                curr_abs_latent = abstraction_latent[f"{t_curr}"].cuda()
                if no_preserve_injection:
                    print("Using abstraction latent for injection")
                    curr_latent = curr_abs_latent
                else:
                    print("Using inverted original latent for injection")
                ...
                if i < abstraction_injection_steps:
                    ...
                    new_sample = (
                        new_sample * (1 - added_deleted_mask)
                        + curr_abs_latent * added_deleted_mask
                    )
                    new_sample = (
                        new_sample * (1 - changed_original_mask)
                        + curr_abs_latent * changed_original_mask
                    )
                ...
                if i < interpolation_steps:
                    ...
                    for edit_mask_data in dilated_edit_masks:
                        ...
                        if transformed_latent is None:
                            print(
                                "transformed latent is None, using abstraction latent"
                            )
                            curr_transformed_latent = curr_abs_latent
                        else:
                            curr_transformed_latent = transformed_latent[
                                f"{t_curr}"
                            ].cuda()
                        # Inject transformed mesh latent into this edit mask region
                        new_sample = (
                            new_sample * (1 - mask_tensor)
                            + curr_transformed_latent * mask_tensor
                        )
                        # new_sample = new_sample * (1 - mask_tensor) + curr_abs_latent * mask_tensor
                        print(
                            f"    Injected transformed latent for SQ {edit_mask_data.get('sq_index')}"
                        )
                ...
                if (
                    inject_original
                    and unchanged_mask is not None
                    and i < abstraction_injection_steps
                ):
                    ...
                    inject_mask = (
                        boundary_mask.unsqueeze(0).unsqueeze(0) * higher_norm_mask
                    )  # Add batch and channel dims

                    # Inject: replace new_sample with curr_latent where mask is active
                    new_sample = (
                        new_sample * (1 - inject_mask) + curr_latent * inject_mask
                    )

                    num_injected = int((inject_mask > 0).sum().item())
                    num_boundary = int((boundary_mask > 0).sum().item())
                    print(
                        f"    Injected original latent into {num_injected} voxels (where norm higher, out of {num_boundary} boundary)"
                    )
...
def run_inpainting(
    ...
    if abstraction_inversion_data is not None:
        if isinstance(abstraction_inversion_data, (str, Path)):
            abstraction_inversion_data = torch.load(
                abstraction_inversion_data, weights_only=False
            )
    ...
    print(
        f"Running inpainting (steps={num_inpainting_steps}, interp={interpolation_steps}, dilation={edit_dilation_steps}, abs_inject={abstraction_injection_steps}, inject_original={inject_original}, cfg={cfg_strength})..."
    )
    ...
    if render_fn is not None:
        ...
        try:
            ...
            render_fn(
                str(output_ply),
                str(output_dir / "inpainted_voxels.png"),
