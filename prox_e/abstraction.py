...
def _get_superdec_model(device: str = None):
    ...
    checkpoints_dir = REPO_ROOT / 'submodules' / 'superdec' / 'checkpoints' / 'normalized'
...
def generate_abstraction(
    ...
    with torch.no_grad():
        ...
        points_tensor = denormalize_points(points_tensor.cpu(), translation_arr, scale_arr, z_up=False)
...
def mesh_from_abstraction(abstraction: List[Dict], resolution: int = 30) -> trimesh.Trimesh:
    """
...
def render_multiview(
    ...
    **render_kwargs
    ...
    if single_view:
        render_path = output_folder / f"{prefix}render.png" if prefix else output_folder / "render.png"
...
class _AbstractionHandler(PredictionHandler):
    ...
    def mesh_from_abstraction(abstraction: List[Dict], resolution: int = 30) -> trimesh.Trimesh:
        ...
        return trimesh.Trimesh(vertices, faces, face_colors=f_colors, vertex_colors=v_colors)
