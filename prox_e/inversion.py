...
from prox_e.appearance_editing import render_for_conditioning, preprocess_single_image, load_trellis_image_pipeline
...
def extract_features_single_thread(output_dir, model="dinov2_vitl14_reg", batch_size=10):
    """
    ...
    dinov2_repo_path = os.path.expanduser("~/.cache/torch/hub/facebookresearch_dinov2_main")
    if os.path.exists(dinov2_repo_path):
        dinov2_model = torch.hub.load(dinov2_repo_path, model, source='local', pretrained=True)
    else:
    ...
    for center_crop_ratio in [None, 0.5]:
        ...
        for view in tqdm(frames, desc="Loading images"):
            ...
            alpha = image[:, :, 3] if image.shape[2] == 4 else np.ones_like(image[:, :, 0])
            ...
            data.append({"image": image, "extrinsics": extrinsics, "intrinsics": intrinsics})

        if skipped_blank > 0:
            print(f"Skipped {skipped_blank} blank images (< 1% alpha coverage)")
        if len(data) > 0:
            if center_crop_ratio is not None:
                print("WARNING: Used center-crop fallback (50% crop) for blank renders.")
            break
    ...
    if len(data) < 10:
        print(f"WARNING: Only {len(data)} valid images out of {len(frames)} - feature quality may be poor")
    ...
    with torch.no_grad():
        for i in tqdm(range(0, n_views, batch_size), desc="Processing image batches"):
            ...
            del features, batch_images, batch_extrinsics, batch_intrinsics, patchtokens, uv
            torch.cuda.empty_cache()
    ...
    # Save features
    pack["patchtokens"] = (
        F.grid_sample(patchtokens, uv.unsqueeze(1), mode="bilinear", align_corners=False)
...
class ConfigurableStepsSampler(InversionFlowEulerGuidanceIntervalSampler):
    ...
    @torch.no_grad()
    def sample(self, model, stage, noise, cond, cfg_strength, latent=None, latent_mask=None, 
               kv=None, self_kv_mask=None, cross_kv_mask=None, skip_step=None, noise_init=None, 
               is_text=False, skip_latent_replacement=True, full_mask=True):
        ...
        for t_curr, t_prev in tqdm(t_pairs, desc=desc, disable=False):
            ...
            sample = self.sample_once(model, sample, t_curr, t_prev, cond, cfg_strength, kv, 
                                       self_kv_mask, cross_kv_mask, t_latent, is_text)
            if inverse_bool:
                latent[f"{t_latent}"] = sample.cpu()
        return sample, latent, kv


def sample_sparse_structure_inverse_configurable(pipeline, cond_src, voxel_src, 
                                                   cfg_strength_stage_1_inverse, skip_step, 
                                                   is_text, num_steps: int = 12):
    """
    ...
    noise, ss_latent, ss_kv = sparse_structure_sampler.sample(
        flow_model, stage, z_s, cond_src, cfg_strength, skip_step=skip_step, is_text=is_text
    )
    return noise, ss_latent, ss_kv


def sample_sparse_structure_denoise_configurable(pipeline, cond_tgt, noise, voxel_src, voxel_mask,
                                                   ss_latent, ss_latent_mask, ss_kv, ss_self_kv_mask,
                                                   ss_cross_kv_mask, cfg_strength_stage_1_forward,
                                                   skip_step, re_init, is_text, 
                                                   skip_latent_replacement=True, full_mask=True,
                                                   num_steps: int = 12):
    """
    ...
    z_s, ss_latent, ss_kv = sparse_structure_sampler.sample(
        flow_model, stage, noise, cond_tgt, cfg_strength, ss_latent, ss_latent_mask, 
        ss_kv, ss_self_kv_mask, ss_cross_kv_mask, skip_step, noise_init, 
        is_text=is_text, skip_latent_replacement=skip_latent_replacement, full_mask=full_mask
    )
...
def sample_slat_inverse_configurable(pipeline, cond_src, slat_src, coords_mask, 
                                      cfg_strength_stage_2_inverse, is_text, num_steps: int = 25):
    """
...
def sample_slat_denoise_configurable(pipeline, cond_tgt, coords_tgt, slat_src, coords_mask, 
                                      slat_latent, slat_kv, slat_self_kv_mask, slat_cross_kv_mask, 
                                      cfg_strength_stage_2_forward, is_text, 
                                      skip_latent_replacement=True, full_mask=True, num_steps: int = 25):
    """
    ...
    slat, slat_latent, slat_kv = slat_sampler.sample(
        flow_model, stage, noise, cond_tgt, cfg_strength, slat_latent, coords_mask, 
        slat_kv, slat_self_kv_mask, slat_cross_kv_mask, is_text=is_text, 
        skip_latent_replacement=skip_latent_replacement, full_mask=full_mask
    )
...
def run_3d_rendering(input_model_path: str, render_dir: str, **render_kwargs) -> dict:
    ...
    print(f"Rendering parameters: num_views={default_params['num_views']}, resolution={default_params['resolution']}")
...
def run_feature_extraction(render_dir: str, max_retries: int = 5, **feature_kwargs) -> dict:
    """
    ...
    print(f"Feature extraction parameters: model={default_params['model']}, batch_size={default_params['batch_size']}")
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if os.path.exists(features_path):
                ...
                print(f"Feature extraction failed - features file not created (attempt {attempt}/{max_retries})")
        except Exception as e:
        ...
        if attempt < max_retries:
            ...
            raise RuntimeError(f"Feature extraction failed after {max_retries} attempts. See {error_log_path}")
...
def run_full_inversion(
    ...
    pipeline = text_pipeline if text_pipeline is not None else load_trellis_text_pipeline()
    ...
    if merging_data is not None and 'changed_edited_masks' in merging_data:
        for mask_data in merging_data['changed_edited_masks']:
            ...
            if transformed_mesh_path and os.path.exists(transformed_mesh_path):
                ...
                _total_structure_time += _inv_result.get('timing', {}).get('structure_inversion', 0)
            ...
            for i, mask_data in enumerate(merging_data['changed_edited_masks']):
                if 'mask_voxel' in mask_data:
                    ...
                    if combined_voxels is not None and mask.shape == combined_voxels.shape:
                        ...
                        print(f"  Added edited_mask_{sq_index}: {int(mask.sum().item())} voxels")
    ...
    if combined_voxels is not None:
        ...
        save_voxels_as_ply(combined_voxels_5d, str(combined_ply), normalization=global_norm)
        print(f"  Saved combined debug mesh: {combined_ply}")
        print(f"  Total voxels in combined mesh: {int((combined_voxels > 0.5).sum().item())}")
...
def run_voxhammer_inversion(
    ...
    voxel_src = voxelize_mesh(mesh_path, center=normalization_center, scale=normalization_scale)
    ...
    if verify:
        ...
        if render_fn is not None:
            try:
                render_fn(str(original_ply), str(inversion_dir / f"{filename}_original_voxels.png"), rotation=trellis_rotation)
                render_fn(str(recon_ply), str(inversion_dir / f"{filename}_reconstructed_voxels.png"), rotation=trellis_rotation)
            except Exception as e:
    ...
    if run_features and os.path.exists(features_path):
        ...
        render_for_conditioning(str(mesh_path), str(conditioning_image_path), shade_smooth=shade_smooth)
        preprocessed_image = preprocess_single_image(str(conditioning_image_path))

        # Extract SLAT from features
        print(f"Extracting SLAT from features...")
        ...
        coords_src = slat_src.coords[:, 1:].int().cuda()  # Remove batch dim, keep [x, y, z]
        ...
        torch.save({
            'slat_noise': slat_noise.cpu() if hasattr(slat_noise, 'cpu') else slat_noise,
        ...
        try:
            ...
            glb_orig = postprocessing_utils.to_glb(
                assets_orig["gaussian"][0], assets_orig["mesh"][0], 
            ...
            if os.path.exists(transforms_path):
                ...
                print(f"    Applied renderer denormalization (scale={renderer_scale:.4f})")
            else:
                print(f"    Warning: transforms.json not found, skipping denormalization")
        ...
        if verify:
            ...
            coords_tgt = torch.cat([torch.zeros(coords_src.shape[0], 1).int().cuda(), coords_src], dim=1)
            ...
            try:
                ...
                glb_recon = postprocessing_utils.to_glb(
                    assets_recon["gaussian"][0], assets_recon["mesh"][0], 
                    simplify=0.95, texture_size=1024
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--steps", type=int, default=25, help="Number of inversion steps (default 12)")
