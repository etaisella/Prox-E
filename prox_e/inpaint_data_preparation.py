    ...
    for idx in orig_by_idx:
        if idx in edit_by_idx:
            ...
            if not exponents_match(sq_orig, sq_edit, tolerance):
                print(f"  [compare_abstractions] SQ {idx} has changed exponents: "
                      f"{sq_orig['exponents']} -> {sq_edit['exponents']}")
                print(f"    -> Treating as deleted (idx {idx}) + added (new idx {next_index})")
...
def points_inside_superquadric(points: np.ndarray, sq: dict, margin: float = 1.1) -> np.ndarray:
    """
...
def make_normalized_to_original(global_center: np.ndarray, global_scale: float) -> np.ndarray:
    """
...
def make_original_to_normalized(global_center: np.ndarray, global_scale: float) -> np.ndarray:
    """
...
def create_mesh_from_superquadrics(superquadrics: List[dict], resolution: int = 30) -> Optional[trimesh.Trimesh]:
...
def voxelize_mesh(mesh: Union[trimesh.Trimesh, o3d.geometry.TriangleMesh], resolution: int = 64) -> np.ndarray:
...
def voxels_to_cube_mesh(
    ...
    template = o3d.geometry.TriangleMesh.create_box(width=voxel_size, height=voxel_size, depth=voxel_size)
    ...
    for i, idx in enumerate(occupied):
        ...
        pos = np.array([grid_pos[0], -grid_pos[2], grid_pos[1]])  # Axis swap for rendering
...
def compute_global_normalization(
    ...
    reference_mesh = create_mesh_from_superquadrics(all_superquadrics, resolution=mesh_resolution)
...
def prepare_edit_data(
    ...
    if create_transformed_meshes and changed_orig_list and changed_edit_list:
        if original_mesh_path is None:
            raise ValueError("original_mesh_path is required when create_transformed_meshes=True")
        ...
        for i, (sq_orig, sq_edit) in enumerate(zip(changed_orig_list, changed_edit_list)):
            ...
            sq_color = sq_edit.get('color', [128, 128, 128])  # Use color from abstraction JSON
            print(f"  SQ {sq_index}: orig_trans={sq_orig['translation']}, edit_trans={sq_edit['translation']}, color={sq_color}")
            
            # === Individual fully-transformed mesh ===
            # Transform ALL vertices
            vertices_homo = np.hstack([original_vertices, np.ones((len(original_vertices), 1))])
            transformed_vertices = (M_transform @ vertices_homo.T).T[:, :3]
            
            # Save individual transformed mesh
            individual_mesh = trimesh.Trimesh(vertices=transformed_vertices.copy(), faces=original_mesh.faces)
            individual_mesh_path = transformed_meshes_folder / f"transformed_mesh_sq{sq_index}.obj"
            ...
            transformed_mesh = trimesh.Trimesh(vertices=transformed_vertices, faces=original_mesh.faces)
            ...
            sq_edit_mesh = create_mesh_from_superquadrics([sq_edit], resolution=mesh_resolution)
            if sq_edit_mesh is not None:
                sq_edit_mesh.vertices[:, 0] += x_offset
                visual_type = type(sq_edit_mesh.visual).__name__
                has_colors = hasattr(sq_edit_mesh.visual, 'vertex_colors') and sq_edit_mesh.visual.vertex_colors is not None
                print(f"    SQ mesh visual type: {visual_type}, has vertex colors: {has_colors}")
                if has_colors:
                    print(f"    First vertex color: {sq_edit_mesh.visual.vertex_colors[0]}")
                all_meshes.append(sq_edit_mesh)
            
            # === Piecewise warping: only vertices inside this sq ===
            inside_mask = points_inside_superquadric(original_vertices, sq_orig, margin=1.5)
        ...
        print(f"  Piecewise total: {np.sum(vertex_warped)} / {len(original_vertices)} vertices warped")
        
        # Save piecewise mesh
        piecewise_mesh = trimesh.Trimesh(vertices=piecewise_vertices, faces=original_mesh.faces)
        ...
        if hasattr(combined_mesh.visual, 'vertex_colors') and combined_mesh.visual.vertex_colors is not None:
    ...
    if external_center is not None and external_scale is not None:
        ...
        abstraction_mesh = create_mesh_from_superquadrics(edited, resolution=mesh_resolution)
    ...
    _, _, reference_mesh = compute_global_normalization(original, edited, mesh_resolution)
    ...
    for i, (sq_orig, sq_edit) in enumerate(zip(changed_orig_list, changed_edit_list)):
        ...
        if save_visualizations:
            ...
            if occupied > 0:
                ...
                o3d.io.write_triangle_mesh(str(mesh_path), cube_mesh, write_vertex_colors=True)
                if render_fn:
                    try:
    ...
    for sq in changed_orig_list:
        ...
        sq_voxels_latent = voxelize_mesh(sq_mesh, resolution=latent_resolution)
        sq_voxels_voxel = voxelize_mesh(sq_mesh, resolution=voxel_resolution)
        changed_original_mask = np.logical_or(changed_original_mask, sq_voxels_latent).astype(np.float32)
        changed_original_mask_voxel = np.logical_or(changed_original_mask_voxel, sq_voxels_voxel).astype(np.float32)
    ...
    for sq in added_deleted_list:
        ...
        sq_voxels_latent = voxelize_mesh(sq_mesh, resolution=latent_resolution)
        sq_voxels_voxel = voxelize_mesh(sq_mesh, resolution=voxel_resolution)
        added_deleted_mask = np.logical_or(added_deleted_mask, sq_voxels_latent).astype(np.float32)
        added_deleted_mask_voxel = np.logical_or(added_deleted_mask_voxel, sq_voxels_voxel).astype(np.float32)
    ...
    for sq in unchanged_list:
        ...
        sq_voxels_latent = voxelize_mesh(sq_mesh, resolution=latent_resolution)
        sq_voxels_voxel = voxelize_mesh(sq_mesh, resolution=voxel_resolution)
        unchanged_mask = np.logical_or(unchanged_mask, sq_voxels_latent).astype(np.float32)
        unchanged_mask_voxel = np.logical_or(unchanged_mask_voxel, sq_voxels_voxel).astype(np.float32)
    ...
    if run_verification and changed_edited_masks:
        _run_latent_verification(
            changed_orig_list, changed_edit_list, changed_edited_masks,
            global_center, global_scale, latent_resolution, mesh_resolution,
            masks_folder, render_fn
        )
    ...
    changed_original_mask_voxel = transform_voxels_to_trellis(changed_original_mask_voxel)
...
def _run_latent_verification(
    ...
    for sq_data in changed_edited_masks:
        edit_voxels_union = np.logical_or(edit_voxels_union, sq_data['mask']).astype(np.float32)
    ...
    for i, idx in enumerate(occupied):
        ...
        if 0 <= i_orig < latent_resolution and 0 <= j_orig < latent_resolution and 0 <= k_orig < latent_resolution:
    ...
    o3d.io.write_triangle_mesh(str(transformed_path), transformed_mesh, write_vertex_colors=True)
    ...
    for i, idx in enumerate(occupied):
        ...
        color = interpolate_grid_values(
            idx.reshape(1, 3).astype(np.float32),
            color_grid,
            M_latent,
            order=1
        )[0]
    ...
    if black_voxels:
        print(f"  First 5 black voxels:")
        for d in black_voxels[:5]:
            print(f"    Voxel {d['voxel_index']}: pos={d['original_position']} -> {d['transformed_position']}, in_bounds={d['in_bounds']}")
    
    interp_mesh = voxels_to_cube_mesh(edit_voxels_union, colors=np.array(interpolated_colors))
    ...
    if len(orig_occupied) > 0:
        ...
        o3d.io.write_triangle_mesh(str(orig_color_path), orig_color_mesh, write_vertex_colors=True)
...
def get_latent_merging_data(
    ...
    """Deprecated: Use prepare_edit_data instead."""
    print("Warning: get_latent_merging_data is deprecated, use prepare_edit_data instead")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Prepare edit data from abstraction comparisons")
    parser.add_argument("--original", type=str, required=True, help="Path to original abstraction JSON")
    parser.add_argument("--edited", type=str, required=True, help="Path to edited abstraction JSON")
    parser.add_argument("--output", type=str, required=True, help="Output folder")
    parser.add_argument("--mesh", type=str, default=None, help="Original mesh path (for transformed meshes)")
    ...
    parser.add_argument("--transform-meshes", action="store_true", help="Create transformed meshes")
