...
from voxhammer.edit_pipeline import InversionFlowEulerGuidanceIntervalSampler, slat_flow_forward, slat_trsfmr_forward, slat_attn_forward
...
def apply_denormalization_to_glb(glb_path: str, center: np.ndarray, scale: float, output_path: str = None):
    """
...
def preprocess_single_image(img_path, remove_background=False, skip_rembg=False) -> Image.Image:
    """
...
class AppearanceEditingSampler(InversionFlowEulerGuidanceIntervalSampler):
    ...
    @torch.no_grad()
    def sample_appearance_editing(self, model, noise, cond, cfg_strength, slat_latent=None, 
                                   preserve_coords_mask=None, injection_steps: int = 0,
                                   edit_injection_mappings=None, added_deleted_coords_mask=None,
        ...
        for i, (t_curr, t_prev) in enumerate(tqdm(t_pairs, desc="Appearance Editing")):
            sample = self.sample_once(model, sample, t_curr, t_prev, cond, cfg_strength, kv, 
                                       None, None, t_curr, False)
            
            # Inject inverted SLAT latents at preserved (unchanged) coordinates
            if i < injection_steps and slat_latent is not None and preserve_coords_mask is not None:
                t_latent = t_curr
                ...
                if latent_key in slat_latent:
                    ...
                    match_1 = (sample.coords.unsqueeze(1) == preserve_coords_mask.unsqueeze(0)).all(dim=-1)
                    match_2 = (slat_latent[latent_key].coords.cuda().unsqueeze(1) == preserve_coords_mask.unsqueeze(0)).all(dim=-1)
                    ...
                    if valid_mask.sum() > 0:
                        ...
                        feats[idx_1[valid_mask]] = slat_latent[latent_key].feats.cuda()[idx_2[valid_mask]]
                    ...
                    if edit_injection_mappings is not None:
                        ...
                        for mapping in edit_injection_mappings:
                            ...
                            if len(sample_indices) > 0:
                                ...
                                sample = sample.replace(feats)
                                print(f"  Edit region SQ{sq_index}: {len(sample_indices)} coords")
                    
                    # Inject into added/deleted regions (abstraction injection)
                    if i < abstraction_injection_steps and added_deleted_coords_mask is not None:
                        # Find matching coordinates between sample and added_deleted mask
                        match_sample = (sample.coords.unsqueeze(1) == added_deleted_coords_mask.unsqueeze(0)).all(dim=-1)
                        match_latent = (slat_latent[latent_key].coords.cuda().unsqueeze(1) == added_deleted_coords_mask.unsqueeze(0)).all(dim=-1)
                        ...
                        if valid_mask.sum() > 0:
                            print(f"  Added/deleted injection: {valid_mask.sum()} coords")
                            feats = sample.feats.clone()
                            feats[idx_sample[valid_mask]] = slat_latent[latent_key].feats.cuda()[idx_latent[valid_mask]]
                            sample = sample.replace(feats)
...
def run_appearance_editing(
    ...
    if appearance_injection_steps > 0 and merging_data is not None and slat_latent is not None:
        print(f"Computing appearance injection mask (injection_steps={appearance_injection_steps})...")
        ...
        edit_union = np.zeros((voxel_resolution, voxel_resolution, voxel_resolution), dtype=np.float32)
        ...
        changed_original = merging_data.get('changed_original_mask_voxel', 
                                             merging_data.get('changed_original_mask', np.zeros((voxel_resolution,)*3)))
        if edit_dilation_steps > 0:
            changed_original = binary_dilation(changed_original > 0.5, iterations=edit_dilation_steps).astype(np.float32)
        edit_union = np.maximum(edit_union, changed_original)
        
        # Add added_deleted mask (dilated) - use voxel resolution version
        added_deleted = merging_data.get('added_deleted_mask_voxel',
                                          merging_data.get('added_deleted_mask', np.zeros((voxel_resolution,)*3)))
        if edit_dilation_steps > 0:
            added_deleted = binary_dilation(added_deleted > 0.5, iterations=edit_dilation_steps).astype(np.float32)
        ...
        if len(preserve_voxel_coords) > 0:
            ...
            print(f"  Preserve mask: {len(preserve_coords_mask)} coordinates (outside edit regions)")
            ...
            print("  Warning: No coordinates to preserve (entire shape is in edit region)")
    ...
    if do_edit_injection and appearance_injection_steps > 0 and merging_data is not None and slat_latent is not None:
        print(f"[Edit Injection ENABLED] Computing edit region injection mappings...")
        from scipy.ndimage import binary_dilation
        voxel_resolution = merging_data.get('voxel_resolution', 64)
        ...
        latent_coords = slat_latent[first_key].coords.cuda()  # (K, 4) with [batch, x, y, z]
        ...
        latent_coords_world = latent_coords_normalized * global_scale + torch.from_numpy(global_center).float().cuda()
        ...
        for edit_mask_info in merging_data.get('changed_edited_masks', []):
            ...
            if M_transform_inv is None:
                print(f"  Warning: M_transform_inv not found for SQ{sq_index}, skipping")
                continue
            
            # Dilate the mask (same as for other masks)
            if edit_dilation_steps > 0:
                mask_voxel = binary_dilation(mask_voxel > 0.5, iterations=edit_dilation_steps).astype(np.float32)
            ...
            match_sample = (coords_tgt.cuda().unsqueeze(1) == edit_coords_with_batch.unsqueeze(0)).all(dim=-1)
            ...
            # normalized → world
            coords_world = coords_normalized * global_scale + torch.from_numpy(global_center).float().cuda()
            ...
            diff = transformed_world.unsqueeze(1) - latent_coords_world.unsqueeze(0)  # (M, K, 3)
            ...
            print(f"  Edit mask SQ{sq_index}: {len(sample_indices)} coords → precomputed NN mapping")
            ...
            for j in range(num_debug):
                ...
                print(f"      [{j}] voxel {src_voxel} → world {src_world} → transformed {tgt_world}")
                print(f"          → NN idx {nn_idx}: voxel {nn_voxel}, world {nn_world}, dist={dist:.4f}")
    ...
    if abstraction_injection_steps > 0 and appearance_injection_steps > 0 and merging_data is not None and slat_latent is not None:
        print(f"Computing added/deleted coords mask (abstraction_injection_steps={abstraction_injection_steps})...")
        from scipy.ndimage import binary_dilation
        voxel_resolution = merging_data.get('voxel_resolution', 64)
    ...
    slat, output_slat_latent, _ = sampler.sample_appearance_editing(
        flow_model, noise, cond, cfg_strength, 
