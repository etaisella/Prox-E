...
ANALYZE_INSTRUCTION_PATH = (
    REPO_ROOT / "instruction_prompts" / "analyze_edit_instruction.txt"
)
IN_CONTEXT_EXAMPLES_PATH = (
    REPO_ROOT / "instruction_prompts" / "in_context_examples.json"
)
...
def _load_analyze_instruction() -> str:
    ...
    if not ANALYZE_INSTRUCTION_PATH.exists():
        raise FileNotFoundError(
            f"Instruction file not found: {ANALYZE_INSTRUCTION_PATH}"
        )
...
def save_parse_result(parse_result: dict, output_path: Path):
    ...
    with open(output_path, "w") as f:
        ...
        f.write(
            f"Structural Description: {parse_result.get('structural_description', '')}\n"
        )
        f.write(
            f"Appearance Description: {parse_result.get('appearance_description', '')}\n\n"
        )
...
def test_on_shapetalk(
    ...
    Returns:
        List of results
    """
    print("Loading ShapeTalk CSV...")
    df = pd.read_csv(_shapetalk_csv_path())

    # Filter for ShapeNet dataset
    filtered = df[
        (df["source_dataset"] == "ShapeNet") & (df["target_dataset"] == "ShapeNet")
    ]
...
def parse_args():
    ...
    parser.add_argument(
        "--category", type=str, default="chair", help="Shape category (default: chair)"
    )
    parser.add_argument(
        "--backend",
        type=str,
...
if __name__ == "__main__":
    ...
    if args.test:
        ...
        if not any(
            [result["appearance_description"], result["structural_description"]]
        ):
            print(f"\n--- Raw Response ---\n{result['raw_response']}")
